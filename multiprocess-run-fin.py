...
def setup_tc_and_iptables(base_port, num_parties, bandwidth_mbps, delay_ms):
    ...
    run_cmd(f"tc class add dev {INTERFACE} parent 1: classid 1:10 htb rate {bandwidth_mbps}mbit ceil {bandwidth_mbps}mbit")
    run_cmd(f"tc qdisc add dev {INTERFACE} parent 1:10 handle 10: netem delay {delay_ms}ms")
    run_cmd(f"tc filter add dev {INTERFACE} parent 1: protocol ip handle 1 fw flowid 1:10")

    for party_id in range(num_parties):
        port = base_port + party_id
        run_cmd(f"iptables -t mangle -A OUTPUT -p tcp --dport {port} -j MARK --set-mark 1")
...
def main():
    if len(sys.argv) != 5:
        print("用法: python3 run_thfhe.py <NUM_PARTIES> <BASE_PORT> <BANDWIDTH_MBPS> <DELAY_MS>")
        sys.exit(1)
    ...
    try:
        for party_id in range(num_parties):
            ...
            print(f"🚀 Started party {party_id} (PID={proc.pid}) on port {base_port + party_id}")
